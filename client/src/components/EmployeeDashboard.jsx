    ...
    <div className='animate-fade-in'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6'>
            {cards.map((card, index)=>(
                <div key={index} className='card card-hover p-4 relative overflow-hidden group flex items-center justify-between'>
                    <div>
                        <div className='absolute left-0 top-0 bottom-0 w-1 rounded-r-full bg-slate-500/70 group-hover:bg-indigo-500/70'/>
                        <p className='text-[13px] font-medium text-slate-700'>{card.title}</p>
                        <p className='text-xl font-bold text-slate-900 mt-0.5'>{card.value}</p>
                    </div>
                    <card.icon className='size-9 p-2 rounded-lg bg-slate-100 text-slate-600 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors duration-200' />
                </div>
            ))}
        </div>
