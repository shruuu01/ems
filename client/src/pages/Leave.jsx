    ...
    <div className="animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        ...
        {!isAdmin && (
          ...
          {leaveStats.map((s)=>(
            <div key={s.label} className="card card-hover p-4 sm:p-6 flex items-center gap-3 relative overflow-hidden group">
              <div className="absolute left-0 top-0 bottom-0 w-0.5 rounded-r-full bg-slate-500/70 group-hover:bg-indigo-500/70" />
              <div className="p-2.5 bg-slate-100 rounded-lg group-hover:bg-indigo-50 transition-colors duration-200">
                <s.icon className="w-4 h-4 text-slate-600 group-hover:text-indigo-600 transition-colors duration-200" />
                </div>
                <div>
                  <p className="text-xs text-slate-500">{s.label}</p>
                  <p className="text-xl font-bold text-slate-900 tracking-tight">{s.value} <span className="text-xs font-normal text-slate-400">taken</span></p>
                </div>
              </div>
